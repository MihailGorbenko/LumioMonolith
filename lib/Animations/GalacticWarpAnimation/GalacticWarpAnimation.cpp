...
void GalacticWarpAnimation::render() {
	...
	for (int x = 0; x < w; ++x) {
		...
		for (int y = 0; y < hgt; ++y) {
			...
			uint8_t spiralHue = (uint8_t)(hue + (t >> 1) + dist * 4 + y * 64);
	...
	int coreX = centerX;
	uint8_t coreBrightness = 255 - (uint8_t)((now / 3) & 0xFF);
	uint8_t coreHue = (uint8_t)(hue + (t >> 1));
	for (int y = 0; y < hgt; ++y) {
		matrix->setPixelHSV(coreX, y, coreHue, sat, coreBrightness);
	}
